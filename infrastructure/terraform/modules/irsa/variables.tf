variable "cluster_oidc_issuer_url" {
  description = "OIDC issuer URL — from eks/ output, used to build the trust policy condition"
  type        = string
}

variable "oidc_provider_arn" {
  description = "ARN of the OIDC provider created in eks/ — passed in so this module never recreates it"
  type        = string
}
